...
| 3 | done 2026-09-13. A manifest that declares no task, or only hidden ones, is said so by `list`, `run` and `capabilities` with the declaration's shape and `kuu docs task`; a bare word that is no verb and no file names both and `--help`; `TASK unknown` for the task the command named suggests the nearest declared one (`_nearest.lua`, shared with `check`) and names `kuu list`, and a wrong `task.default` is reported as the manifest's own; `TASK noproject` names adopting; the first crossing that creates `.kuu/` under a root no `.gitignore` up to the repository's ignores it in says so once; `check` finds the 0.8 version guard — two components matched to the end, in either spelling — as a `value` error naming `version_at_least` and upgrading-0.9, and warns from a tasks.lua project; `notes` on the run, list, dry-run, check and capabilities envelopes. A two-lens review confirmed eleven (hidden-only manifests, the pattern rule over-firing, gitignore spellings and parents, the note naming the wrong file, the wrong default) — all folded in before the gate |
| 4 | done 2026-09-13. ledger.md's `LedgerRecord` type with the error shape and a reader of the last failed task; `TASK failed` carries `status` and `limit` and the child event `limit`; err.md's "In your own code"; four front-door recipes (a manifest with a declared tool, a wrapper module decoding NDJSON, a reader of the `--json` stream, a reader of the ledger), exercised by the cookbook case; task.md says a returned string is wrapped, a child without a timeout has no bound, and `kuu run TASK --help` prints the task's usage and exits 0 — for a task with no arguments too; mem.md's `.kuu/` exception gone; pitfalls.md names every stderr shape; check.md says what a non-parsing file gets. A two-lens review confirmed fifteen (git's optional head, `exit` the one field that travels, the readers naming the task not a child, the verb record's comments, the `--help` of a task with no arguments, the recipe's exit-code rule, `--help` under `--json`) — all folded in before the gate |
| 5 | done 2026-09-13. `KUU_VERSION` 0.10.0; `kuu --help` carries every verb's options and is the one usage string, written by `bundle_docs --write` between markers in index.md, README.md and kuu.md's Start here and held by the bundle case; index.md's version sentence and inventory, stability's and adopting's guards at 0.10, upgrading-0.10's preamble and a section on the executable explaining itself, kuu.md's Where things stand and a Where 0.10.0 stands, README's version paragraph, the roadmap's landed rows. Pushed, tagged `0.10.0` on `876b143`, signed with the owner's Certum certificate and published on 2026-09-13 by `make publish`, which repeated the gate before signing; the downloaded asset, its sidecar and the local signed file agree, and the released binary reports its own certificate identity |
