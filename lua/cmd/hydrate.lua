-- hydrate.lua -- `kuu hydrate [--deep] [--lock FILE] [--root DIR] [--json]`
global none
...
local spec = {
  { "--deep", type = "flag", help = "also re-hash every installed executable and notice" },
...
local opts, e = cli.parse(rt.args, spec, "kuu hydrate")
if not opts then io.stderr:write("kuu: ", tostring(e), "\n") os.exit(2) end
if opts.help then io.write(cli.usage(spec, "kuu hydrate")) os.exit(0) end
...
local function finish(ok, e2, report, code)
  ...
  elseif not ok then
    ...
    for _, t in ipairs(report.tools) do
      io.write(string.format("%-9s %-16s %-12s %s\n", t.action, t.name, tostring(t.version or ""), t.path))
    end
  end
...
local report, e4, partial = toolchain.hydrate(lock, root, {
  deep = opts.deep,
  progress = function(step, name, detail)
    if not opts.json then io.stderr:write("kuu: ", step, " ", name, ": ", detail, "\n") end
  end,
})
if not report then finish(false, e4, partial, 1) end
