...
| an interactive tool that reads console prompts | `pty.spawn { "cmd.exe" }`, `p:expect({ ">" }, "5s")`, `p:write("echo hello\r")` | a private pseudoconsole with supervised lifetime; provisional API, see [pty](pty.md) |
...
| `$env:NAME`, `[Environment]::SetEnvironmentVariable(..., 'User')`, `setx` | `env.get`, `env.set`; `env.persist`, `env.forget` with the change broadcast | live and persisted are two different things; see [env](env.md) |
